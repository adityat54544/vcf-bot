#!/usr/bin/env python3
# Aura VCF Bot — full rewritten, callback fixes
import os
import re
import uuid
import time
...
BOT_TOKEN = "7644164685:AAG1b0fmrR-iOS6j5q535YK34P42722s6cY"
...
async def keep_alive_task(bot):
    """
    Silent keep-alive task that runs periodically to prevent VPS from killing inactive bots.
    Completely non-intrusive - only uses logging and minimal API calls, no Telegram messages.
    """
    ping_counter = 0
    while True:
        try:
            ...
            # Log activity to show the process is alive (visible in console/logs only)
            logger.info("🔄 SILENT PING #%d: Bot is running and healthy", ping_counter)

            # Perform minimal Telegram API call to maintain connection (no messages sent)
            try:
                me = await bot.get_me()
                logger.debug("🤖 Connection verified for bot @%s", me.username)
            except Exception as e:
                logger.warning("⚠️ Connection check failed: %s", e)

            # Sleep for the configured interval (3 minutes)
            ...
            logger.error("💥 Keep-alive task error: %s", e)
...
def main():
    try:
        ...
        logger.info("Aura VCF Bot running (callbacks fixed)")

        # Start keep-alive task if enabled
        if KEEP_ALIVE_ENABLED:
            logger.info("Keep-alive task started (interval: %s seconds)", KEEP_ALIVE_INTERVAL)
...
To deploy this bot on PythonAnywhere (www.pythonanywhere.com):

1. UPLOAD FILES:
   - Upload simple_vcf_bot.py to your PythonAnywhere account
   - Upload requirements.txt

2. SET UP VIRTUAL ENVIRONMENT:
   mkvirtualenv --python=/usr/bin/python3.8 aura_vcf_bot
   workon aura_vcf_bot
   pip install -r requirements.txt

3. CONFIGURE ENVIRONMENT VARIABLES:
   - Go to PythonAnywhere Web tab → Environment variables
   - Add: BOT_TOKEN="your_telegram_bot_token_here"
   - Add: REQUIRED_CHANNELS='[{"username": "@aurabots0", "invite_url": "https://t.me/+_RfSmS5WOOM3MGRl"}, {"username": "@workbyaditya", "invite_url": "https://t.me/workbyaditya"}, {"username": "@aurachatsws", "invite_url": "https://t.me/aurachatsws"}]'

4. START THE BOT:
   nohup python simple_vcf_bot.py > bot.log 2>&1 &

5. MONITOR THE BOT:
   tail -f bot.log
   ps aux | grep simple_vcf_bot

6. RESTART THE BOT:
   pkill -f "python simple_vcf_bot.py"
   nohup python simple_vcf_bot.py > bot.log 2>&1 &

MIGRATION SUMMARY:
- Removed all Replit-specific code (.replit, replit.py)
- Removed Flask dependency from requirements.txt
- Updated main execution for PythonAnywhere compatibility
- Preserved all core bot functionality
- Bot now runs 24/7 on PythonAnywhere without Replit dependencies
"""

if __name__ == "__main__":
    logger.info("🏃 Running Aura VCF Bot on PythonAnywhere")
    main()
